...
from core import createMachine, state, transition, reduce, interpret, invoke, state as final, immediate
...
class TestInvokeAsync(unittest.TestCase):
    ...
    async def test_done(self):
        ...
        self.assertEqual(service.machine.current,
                         'three', 'now in the next state')
...
class TestInvokeMachine(unittest.TestCase):
    def test_child_machine(self):
        ...
        def aux(thisService):
            ...
            match c:
                ...
                case 1:
                    self.assertNotEqual(
                        thisService, service, 'second time a different service')
                case 2:
                    self.assertEqual(service.machine.current,
                                     'three', 'now in three state')
            c += 1
        ...
        def aux(thisService):
            ...
            match c:
                ...
                case 2:
                    ...
                    self.assertEqual(thisService.machine.current,
                                     'three', 'now in three state')
                case 3:
                    self.assertEqual(service.machine.current, 'four')
                case 4:
                    self.assertNotEqual(
                        thisService, service, 'third time a different service')
                    self.assertEqual(thisService.machine.current, 'nestedFour')
                case 5:
                    self.assertEqual(service.machine.current,
                                     'five', 'now in five state')
            c += 1
    ...
    async def test_child_receive_events(self):
        ...
        child = createMachine({
            'init': state(
                immediate('waiting', action(
                    lambda ctx: ctx['stuff'].append(1)))
            ),
    ...
    def test_multi_level(self):
        ...
        def aux(thisService):
            ...
            match c:
                ...
                case 1:
                    self.assertNotEqual(
                        thisService.machine.states, service.machine.states, 'second time a different service')
                    self.assertNotEqual(service.child, None, 'has child')
                    self.assertEqual(service.child.machine.current, 'start')
                case 2:
                    self.assertNotEqual(service.child.child,
                                        None, 'has grand child')
                    self.assertEqual(service.child.machine.current, 'start')
                    self.assertEqual(
                        service.child.child.machine.current, 'start')
                case 3:
                    self.assertNotEqual(
                        service.child.child.child, None, 'has grand grand child')
                    self.assertEqual(
                        service.child.child.machine.current, 'start')
                    self.assertEqual(
                        service.child.child.child.machine.current, 'start')
                case 4:
                    self.assertEqual(
                        service.child.child.child.machine.current, 'done')
                case 5:
                    self.assertEqual(
                        service.child.child.machine.current, 'done')
                    self.assertEqual(service.child.child.child,
                                     None, 'child is removed when resolved')
                case 6:
                    self.assertEqual(service.child.machine.current, 'done')
                    self.assertEqual(service.child.child, None,
                                     'child is removed when resolved')
                case 7:
                    self.assertEqual(service.machine.current, 'done')
                    self.assertEqual(service.child, None,
                                     'child is removed when resolved')
